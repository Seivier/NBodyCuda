...
int main()
{
	...
	std::fstream vertexShaderFile ("shaders/nbody.vert");
	...
	std::fstream geometryShaderFile("shaders/nbody.geom");
	std::string geometryShaderSource((std::istreambuf_iterator<char>(geometryShaderFile)),
		std::istreambuf_iterator<char>());
	...
	std::fstream fragmentShaderFile("shaders/nbody.frag");
	...
	float vertices[] = {
		...
		-0.5f, -0.5f, 0.0f, 0.3f, 0.5f, 0.0f,  // bottom left
...
//	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);
	...
	auto acc = glGetAttribLocation(shaderProgram, "acceleration");
	glVertexAttribPointer(acc, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(3 * sizeof(float)));
	glEnableVertexAttribArray(acc);

	// set uniforms
//	auto apothem = glGetUniformLocation(shaderProgram, "apothem");
//	glUniform1f(apothem, 0.05f);
//
//	auto gen_quads = glGetUniformLocation(shaderProgram, "gen_quads");
//	glUniform1i(gen_quads, 0);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		auto ratio = glGetUniformLocation(shaderProgram, "aspectRatio");
		glUniform1f(ratio, aspect_ratio);
