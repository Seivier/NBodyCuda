...
Buffer::Buffer()
	: _ebo(0), _vbo(0), _vao(0), data(), _ratio(0.0f), _s_mass(MASS), _size(0), _cuda_resource(nullptr)
{
...
//	}
	...
	bind();
	glBufferData(GL_ARRAY_BUFFER, _size * sizeof(Point), data.data(), GL_DYNAMIC_DRAW);
	std::vector<unsigned int> indices(_size);
	for (unsigned int i = 0; i < _size; ++i)
		indices[i] = i;

	glBufferData(GL_ELEMENT_ARRAY_BUFFER, _size * sizeof(unsigned int), indices.data(), GL_DYNAMIC_DRAW);

	cudaGraphicsGLRegisterBuffer(&_cuda_resource, _vbo, cudaGraphicsMapFlagsNone);
...
float* Buffer::getCudaPoints() const 
{
	size_t num_bytes = _size * sizeof(Point);
	cudaGraphicsResourceGetMappedPointer((void**)&_cuda_points, &num_bytes, _cuda_resource);
	return _cuda_points;
}

void Buffer::map()
{
	cudaGraphicsMapResources(1, &_cuda_resource, 0);
}

void Buffer::unmap()
{
	// copy data from cuda to host
	cudaMemcpy(data.data(), _cuda_points, _size * sizeof(Point), cudaMemcpyDeviceToHost);
	cudaGraphicsUnmapResources(1, &_cuda_resource, 0);
}
